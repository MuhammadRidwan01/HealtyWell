...
export function LiveChat() {
  ...
  // Function to load chat sessions - extracted for reuse
  const loadChatSessions = useCallback(() => {
    if (isAuthenticated && user) {
      ...
      // Clear chat sessions when not authenticated
      setChatSessions([])
      setActiveSessionId("")
  ...
  // Create a new chat session - modified to use useCallback
  ...
  // Load chat sessions on component mount and when authentication state changes
  useEffect(() => {
    loadChatSessions()
  }, [loadChatSessions])
  ...
  useEffect(() => {
    if (isAuthenticated && user && chatSessions.length > 0) {
      const storageKey = `chatSessions_${user.id}`
      localStorage.setItem(storageKey, JSON.stringify(chatSessions))
    }
  }, [chatSessions, isAuthenticated, user])

  // Listen for auth state changes to immediately respond
  useEffect(() => {
    // When user logs out, close the chat window and clear sessions
    if (!isAuthenticated) {
      setIsOpen(false)
      setChatSessions([])
      setActiveSessionId("")
    ...
    try {
      ...
      if (data && data.message) {
        ...
        if (data.topic) {
          ...
          const userFirstMessage = userMessage.content.substring(0, 20);
  ...
  // Handle chat button click - check authentication first
  const handleChatButtonClick = () => {
    ...
    // If we're opening the chat and there are no sessions, create one
