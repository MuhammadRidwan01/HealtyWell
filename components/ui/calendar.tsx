...
function YearPicker({ currentYear, onSelect }) {
  const [decade, setDecade] = React.useState(Math.floor(currentYear / 10) * 10)
  
  // Generate years from 1900 to current year + 50
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
        {months.map((month, idx) => (
          <motion.button
            ...
            className={cn(
              "rounded-md py-2 text-sm font-medium",
              idx === currentMonth ? "bg-primary text-primary-foreground" : "hover:bg-accent"
            )}
        ...
        <Popover open={showMonthPicker} onOpenChange={setShowMonthPicker}>
          <PopoverTrigger asChild>
            <motion.button
              ...
              whileTap={{ scale: 0.95 }}
              className="px-3 py-1 rounded-md hover:bg-accent text-sm font-medium flex items-center"
            >
        ...
        <DayPicker
          ...
          components={{
            Caption: ({ displayMonth, ...captionProps }) => (
              <CustomCaption displayMonth={displayMonth} onMonthChange={captionProps.onMonthChange} />
            ),
          }}
          {...props}
        />
