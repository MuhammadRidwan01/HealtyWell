...
export default function ChatPage() {
  ...
  const sendMessage = async () => {
    ...
    try {
      const res = await fetch(`http://localhost:5000/consultations/${id}/messages`, {
        method: "POST",
        headers: {
  ...
  const handleConsultationEnd = async (diagnosisMessage: string) => {
    try {
      ...
      await fetch(`http://localhost:5000/consultations/${id}/end`, {
        method: "POST",
        headers: { Authorization: `Bearer ${token}` },
      });
  
      // Save diagnosis notes only
      await fetch(`http://localhost:5000/consultations/${id}/notes`, {
        method: "PATCH",
        headers: {
          Authorization: `Bearer ${token}`,
          "Content-Type": "application/json",
  ...
  />
                  ...
                  <motion.div
                    ...
                    className={`p-4 rounded-xl shadow-md flex-1 ${
                      msg.role === "user" 
